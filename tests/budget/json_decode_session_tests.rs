...
fn test_failed_second_value_preserves_first_commit_and_accumulates_input() {
    ...
    decode_slice::<IgnoredAny, _, _>(first, &mut session)
        .expect("first value must fit");
    assert!(decode_slice::<IgnoredAny, _, _>(second, &mut session).is_err());
    ...
    decode_slice::<IgnoredAny, _, _>(third, &mut session)
...
fn test_decode_attempt_panic_retains_input_and_reuses_value_capacity() {
    ...
    decode_slice::<IgnoredAny, _, _>(b"null", &mut session)
