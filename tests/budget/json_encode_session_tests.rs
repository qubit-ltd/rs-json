...
#[test]
fn test_encode_to_vec_stream_failure_rolls_back_output_and_value() {
    let mut session = JsonEncodeSession::owned(
    ...
    assert!(encode_to_vec(&FailsAfterPrefix, &mut session).is_err());
...
fn test_buffered_writer_failure_keeps_prefix_and_rolls_back_value() {
    ...
    assert!(
        encode_to_writer(&mut writer, &[1_u8, 2_u8], &mut session).is_err()
    );
