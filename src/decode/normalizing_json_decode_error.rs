...
// =============================================================================
//! Defines the [`NormalizingJsonDecodeError`] type used by the lenient decoder
//! API.
...
impl NormalizingJsonDecodeError {
    ...
    pub(crate) const fn input_too_large(
        ...
        Self {
            privacy_policy,
            failure: NormalizingJsonDecodeFailure::InputTooLarge {
                raw_input_bytes,
                maximum,
            },
        }
    ...
    pub(crate) const fn normalized_input_too_large(
        ...
        Self {
            privacy_policy,
            failure: NormalizingJsonDecodeFailure::NormalizedInputTooLarge {
                raw_input_bytes,
                normalized_input_bytes,
                maximum,
            },
    ...
    pub(crate) const fn empty_input(
        ...
        Self {
            privacy_policy,
            failure: NormalizingJsonDecodeFailure::EmptyInput {
                raw_input_bytes,
                normalized_input_bytes,
            },
        }
    ...
    pub(crate) fn invalid_json(
        ...
        Self {
            privacy_policy,
            failure: NormalizingJsonDecodeFailure::InvalidJson {
                raw_input_bytes,
                normalized_input_bytes,
                line,
                column,
    ...
    pub(crate) const fn budget(
        ...
        Self {
            privacy_policy,
            failure: NormalizingJsonDecodeFailure::Budget {
                raw_input_bytes,
    ...
    pub(crate) const fn unexpected_top_level(
        ...
        Self {
            privacy_policy,
            failure: NormalizingJsonDecodeFailure::UnexpectedTopLevel {
                raw_input_bytes,
                normalized_input_bytes,
                expected,
                actual,
    ...
    pub(crate) fn deserialize(
        ...
        Self {
            privacy_policy,
            failure: NormalizingJsonDecodeFailure::Deserialize {
                raw_input_bytes,
                normalized_input_bytes,
                line,
                column,
    ...
    #[must_use]
    #[inline(always)]
    pub const fn privacy_policy(&self) -> DiagnosticPolicy {
        self.privacy_policy
    }
    ...
    pub const fn normalized_line(&self) -> Option<usize> {
        match self.failure {
            NormalizingJsonDecodeFailure::InvalidJson { line, .. }
            | NormalizingJsonDecodeFailure::Deserialize { line, .. }
                if line > 0 =>
            {
                Some(line)
            }
    ...
    pub const fn normalized_column(&self) -> Option<usize> {
        match self.failure {
            NormalizingJsonDecodeFailure::InvalidJson { column, .. }
            | NormalizingJsonDecodeFailure::Deserialize { column, .. }
                if column > 0 =>
            {
                Some(column)
            }
...
impl PartialEq for NormalizingJsonDecodeFailure {
...
impl Eq for NormalizingJsonDecodeError {}

impl fmt::Debug for NormalizingJsonDecodeError {
    /// Formats structured diagnostics without reconstructing redacted sources.
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("NormalizingJsonDecodeError")
...
impl fmt::Display for NormalizingJsonDecodeFailure {
...
impl fmt::Display for NormalizingJsonDecodeError {
...
impl Error for NormalizingJsonDecodeError {
