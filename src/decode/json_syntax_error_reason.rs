...
impl From<JsonLexicalErrorReason> for JsonSyntaxErrorReason {
    ...
    fn from(reason: JsonLexicalErrorReason) -> Self {
        match reason {
            ...
            JsonLexicalErrorReason::IntegerOutOfRange => Self::IntegerOutOfRange,
            JsonLexicalErrorReason::FloatOutOfRange => Self::FloatOutOfRange,
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::IntegerOutOfRange => {
                formatter.write_str("JSON integer is outside the supported 64-bit range")
            }
            Self::FloatOutOfRange => {
                formatter.write_str("JSON number is outside the finite f64 range")
            }
