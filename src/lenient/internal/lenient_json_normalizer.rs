...
use super::super::LenientJsonDecodeError;
use super::super::LenientJsonDecodeOptions;
...
impl Default for LenientJsonNormalizer {
    ...
    /// # Returns
    ///
    /// A normalizer configured with [`LenientJsonDecodeOptions::default`].
    #[inline(always)]
    fn default() -> Self {
        Self::new(LenientJsonDecodeOptions::default())
    }
}
...
impl LenientJsonNormalizer {
    ...
    #[inline(always)]
    #[must_use]
    pub(crate) const fn new(options: LenientJsonDecodeOptions) -> Self {
        Self { options }
    }
    ...
    pub(crate) const fn options(&self) -> &LenientJsonDecodeOptions {
        &self.options
    }
    ...
    /// # Errors
    ///
    /// Returns [`LenientJsonDecodeError`] when the raw or normalized input
    ...
    ) -> Result<Cow<'a, str>, LenientJsonDecodeError> {
    ...
    /// # Errors
    ///
    /// Returns [`LenientJsonDecodeError`] when the input is empty under the
    /// active whitespace policy.
    fn require_non_empty<'a>(
        &self,
        input: &'a str,
        raw_input_bytes: usize,
    ) -> Result<&'a str, LenientJsonDecodeError> {
        if self.options.trim_whitespace() {
            if input.trim().is_empty() {
                return Err(LenientJsonDecodeError::empty_input(
                    raw_input_bytes,
                    None,
                    self.options.error_privacy_policy(),
                ));
    ...
    fn consume_normalized_input(
        ...
        if attempt
            ...
            .is_err()
        {
            return Err(LenientJsonDecodeError::normalized_input_too_large(
                raw_input_bytes,
