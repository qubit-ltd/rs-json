...
`LenientJsonDecoder` is a reusable object with immutable
`LenientJsonDecodeOptions`. It can remove only configured noise, then
...
`ErrorPrivacyPolicy::Detailed` only where input-derived diagnostics are safe.
...
`JsonTextEncoder::write_buffered` commits only after complete output is ready
...
1. `lenient::LenientJsonDecodeError` for normalization and lenient typed decode.
2. `text::JsonDecodeError` for strict budget, syntax, or typed decode failure.
3. `text::JsonEncodeError` for strict budget, raw JSON, serialization, or I/O failure.
4. `text::JsonSyntaxError` for stable syntax reason and location metadata.
