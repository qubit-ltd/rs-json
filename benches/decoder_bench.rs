...
fn benchmark_decoder(c: &mut Criterion) {
    ...
    let mut strict_session = JsonDecodeSession::owned(JsonDecodeLimits::empty());
    consume_record(
    ...
    comparison.bench_function("strict_decoder", |bencher| {
        bencher.iter(|| {
            ...
            black_box(decoder.decode::<BenchmarkRecord>(black_box(plain_input.as_bytes())))
        });
    });
...
fn benchmark_downstream_scaling(c: &mut Criterion) {
        ...
        plain_group.bench_with_input(
            ...
            |bencher, input| {
                bencher.iter(|| {
                    let mut session = JsonDecodeSession::owned(JsonDecodeLimits::empty());
                    let mut decoder = JsonTextDecoder::new(&mut session);
                    black_box(decoder.decode::<BenchmarkRecord>(black_box(input.as_bytes())))
                });
            },
        );
        plain_group.bench_with_input(
    ...
    let bounded_decoder = LenientJsonDecoder::new(
        LenientJsonDecodeOptions::strict().with_max_input_bytes(Some(plain.len() - 1)),
    );
    assert!(
        default_decoder
    ...
    for payload_bytes in [65_536_usize, 1_048_576] {
        ...
        for (name, input) in [
            ...
            assert!(
                JsonTextDecoder::new(&mut JsonDecodeSession::owned(JsonDecodeLimits::empty()))
                    .decode::<BenchmarkRecord>(input.as_bytes())
                    .is_err(),
                ...
                |bencher, input| {
                    bencher.iter(|| {
                        black_box(
                            ...
                            .decode::<BenchmarkRecord>(black_box(input.as_bytes())),
                        )
                    });
                },
            );
